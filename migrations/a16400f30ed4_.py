...
def _downgrade(table_obj):
    bind = op.get_bind()

    for row in bind.execute(table_obj.select()):
        op.execute(table_obj.delete().where(table_obj.c.id == row.id))


def _downgrade_book(table_obj):
    bind = op.get_bind()

    for row in bind.execute(table_obj.select()):
        op.execute(table_obj.delete().where(table_obj.c.isbn == row.isbn))
